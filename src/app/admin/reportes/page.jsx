...
const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
        return (
            <div className="bg-surface-raised border border-border-subtle rounded-xl px-4 py-3 shadow-lg">
                <p className="text-xs text-text-tertiary mb-1">{label || payload[0].name}</p>
        ...
        <main data-density="compact" className="min-h-screen bg-surface-page px-8 py-8">
            ...
            <div className="flex items-start justify-between mb-8 flex-wrap gap-4">
                <div>
                    ...
                    <p className="text-sm text-text-tertiary mt-1">
                        Mostrando datos: <span className="font-medium text-text-secondary">{periodLabel}</span>
                    </p>
                </div>

                {/* Period Toggle */}
                <div className="flex items-center gap-1 bg-surface-raised border border-border-subtle rounded-2xl p-1.5">
            ...
            {loading && data && (
                <div className="flex items-center gap-2 mb-4 text-sm text-text-tertiary">
                    <div className="w-4 h-4 border-2 border-border-default border-t-text-secondary rounded-full animate-spin" />
                    Actualizando datos...
                </div>
            )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <div className="bg-surface-raised border border-border-subtle rounded-2xl p-6 hover:shadow-md hover:border-border-default transition-all duration-300">
                    <p className="text-xs text-text-tertiary uppercase tracking-wide mb-2">Total Deudores</p>
                    <p className="text-4xl font-bold font-mono text-text-primary">{data.totalDebtors}</p>
                    <p className="text-xs text-text-tertiary mt-2">{periodLabel}</p>
                </div>

                <div className="bg-surface-raised border border-border-subtle rounded-2xl p-6 hover:shadow-md hover:border-border-default transition-all duration-300">
                    <p className="text-xs text-text-tertiary uppercase tracking-wide mb-2">Total Adeudado</p>
                    <p className="text-4xl font-bold font-mono text-text-primary">{formatMoney(data.totalAmountOwed)}</p>
                    <p className="text-xs text-text-tertiary mt-2">{periodLabel}</p>
                </div>

                <div className="bg-surface-raised border border-border-subtle rounded-2xl p-6 hover:shadow-md transition-all duration-300">
                    <p className="text-xs text-text-tertiary uppercase tracking-wide mb-2">Recuperado</p>
                    <p className="text-4xl font-bold font-mono text-success">{formatMoney(data.totalCollected)}</p>
                    <p className="text-xs text-text-tertiary mt-2">{periodLabel}</p>
                </div>

                <div className="bg-surface-raised border border-border-subtle rounded-2xl p-6 hover:shadow-md hover:border-border-default transition-all duration-300">
                    <p className="text-xs text-text-tertiary uppercase tracking-wide mb-2">Tasa de Recuperación</p>
                    <p className="text-4xl font-bold text-success">{recoveryRate}%</p>
                    <p className="text-xs text-text-tertiary mt-2">{periodLabel}</p>
                    <div className="mt-3 w-full h-1.5 bg-border-default rounded-full overflow-hidden">
                        <div
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-6">
                ...
                {/* Bar Chart */}
                <div className="md:col-span-2 bg-surface-raised border border-border-subtle rounded-2xl p-6 hover:shadow-md transition-all duration-300">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="font-semibold text-text-primary">Distribución por Estado</h2>
                        <span className="text-xs text-text-tertiary bg-surface-hover px-3 py-1 rounded-full">{periodLabel}</span>
                    </div>
                    <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={barData} barSize={40}>
                            {/* stroke used to be "var(--color-accent)10" — string-concatenating
                                an alpha suffix onto a var() call isn't valid CSS at all (that
                                trick only works on a literal hex string), so this line never
                                actually rendered a tinted grid; fixed to a real token. */}
                            <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border-subtle)" vertical={false} />
                            <XAxis dataKey="status" tick={{ fontSize: 11, fill: 'var(--color-text-tertiary)' }} axisLine={false} tickLine={false} />
                ...
                <div className="bg-surface-raised border border-border-subtle rounded-2xl p-6 hover:shadow-md transition-all duration-300">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="font-semibold text-text-primary">Métricas del Portafolio</h2>
                        <span className="text-xs text-text-tertiary bg-surface-hover px-3 py-1 rounded-full">{periodLabel}</span>
                    </div>
                    <div className="space-y-5">
                        <div className="p-4 bg-surface-hover rounded-xl">
                            <p className="text-xs text-text-tertiary uppercase tracking-wide mb-1">Portafolio Total</p>
                            <p className="text-xl font-bold font-mono text-text-primary">{formatMoney(data.totalAmountOwed)}</p>
                        </div>
                        <div className="p-4 bg-surface-hover rounded-xl">
                            <p className="text-xs text-text-tertiary uppercase tracking-wide mb-1">Recuperado</p>
                            <p className="text-xl font-bold text-success">{formatMoney(data.totalCollected)}</p>
                        </div>
                        <div className="p-4 bg-surface-hover rounded-xl">
                            <p className="text-xs text-text-tertiary uppercase tracking-wide mb-1">Pendiente</p>
                            <p className="text-xl font-bold font-mono text-text-primary">{formatMoney(outstanding)}</p>
                        </div>
                        <div>
                            ...
                            <p className="text-[10px] text-text-tertiary mt-1 text-right">Promedio sector ~35%</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="bg-surface-raised border border-border-subtle rounded-2xl p-6 hover:shadow-md transition-all duration-300">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="font-semibold text-text-primary">Deudores por Estado</h2>
                        <span className="text-xs text-text-tertiary bg-surface-hover px-3 py-1 rounded-full">{periodLabel}</span>
                    </div>
                ...
                <div className="grid md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div className="bg-surface-raised border border-border-subtle rounded-2xl p-6 hover:shadow-md transition-all duration-300">
                        ...
                        <p className="text-xs text-text-tertiary mt-2">Emails</p>
                        <p className="text-xs text-text-tertiary mt-1">{periodLabel}</p>
                    </div>

                    <div className="md:col-span-1 bg-surface-raised border border-border-subtle rounded-2xl p-6 hover:shadow-md transition-all duration-300">
                ...
                <div className="bg-surface-raised border border-border-subtle rounded-2xl p-6 hover:shadow-md transition-all duration-300">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="font-semibold text-text-primary">Detalle por Estado</h2>
                        <span className="text-xs text-text-tertiary bg-surface-hover px-3 py-1 rounded-full">{periodLabel}</span>
                    </div>
                        ...
                        {Object.entries(data.byStatus).map(([status, count], i) => {
                            ...
                            return (
                                <div key={i} className="flex items-center gap-3">
                                    ...
                                    <span className="text-sm text-text-secondary flex-1">{formatStatus(status)}</span>
