...
export default function ClientPage() {
    ...
    return (
        <div className="min-h-screen bg-white text-[#443CA3] p-10">
            ...
            {/* Debtor Form */}
            <form onSubmit={handleSubmit} className="mt-8 space-y-4 max-w-lg bg-[#CCE8FF] p-6 rounded-2xl shadow-md">
                <h2 className="text-xl font-semibold text-[#443CA3] font-[Neulis Alt]">
                    {editingId ? 'Edit Debtor' : 'Add a Debtor'}
                </h2>

                <input type="text" placeholder="Debtor Name"
                       value={name} onChange={(e) => setName(e.target.value)}
                       className="w-full p-3 border border-[#9E9E9E] rounded-lg focus:ring-2 focus:ring-[#21FE83]" required />

                <input type="email" placeholder="Debtor Email (optional)"
                ...
                <PhoneInput country={'ec'} value={telephone} onChange={(phone) => setTelephone(phone)} enableSearch={true}
                ...
                <input type="text" placeholder="Cédula de Identidad" value={cedulaIdentidad}
                       onChange={(e) => { const v = e.target.value.replace(/\D/g, ''); if (v.length <= 10) setCedulaIdentidad(v); }}
                       className="w-full p-3 border border-[#9E9E9E] rounded-lg" required />
                {cedulaIdentidad.length > 0 && cedulaIdentidad.length < 10 && (
                    <p className="text-red-500 text-sm mt-1">La cédula debe tener 10 dígitos.</p>
                )}

                <input type="text" placeholder="Amount Owed" value={amountOwed}
                       onChange={handleAmountChange} onBlur={handleBlur} onFocus={handleFocus}
                       className="w-full p-3 border border-[#9E9E9E] rounded-lg" required />

                <label className="w-full bg-[#21FE83] text-[#443CA3] font-bold py-3 rounded-lg shadow hover:bg-[#1edb70] transition disabled:opacity-50">
            ...
            <div className="mt-8 flex gap-4">
                ...
                <input type="file" accept=".xlsx" onChange={handleFileUpload}
                       className="block w-64 text-sm text-gray-600
            file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0
            file:text-sm file:font-semibold file:bg-[#CCE8FF] file:text-[#443CA3]
            hover:file:bg-[#b8ddff]" />
            </div>
            {message && <p className="mt-2 text-sm font-semibold text-[#443CA3]">{message}</p>}
            ...
            <div className="mt-10">
                ...
                <h2 className="text-xl font-bold font-[Neulis Alt] mb-4">Your Debtors</h2>
                {debtors.length === 0 ? (
                    <p className="text-[#9E9E9E]">No debtors yet.</p>
                ) : (
                    <ul className="space-y-3">
                        {filteredDebtors.map((debtor) => (
                            <li key={debtor.id}
                                className="border border-[#CCE8FF] p-4 rounded-xl bg-white shadow-md flex justify-between items-start">
                                <div className="space-y-1 text-sm font-[Avenir Next] text-[#443CA3]">
                                    <p><span className="font-bold">Name:</span> {debtor.name}</p>
                                    {debtor.email && <p><span className="font-bold">Email:</span> {debtor.email}</p>}
                                    {debtor.telephone && <p><span className="font-bold">Telephone:</span> {debtor.telephone}</p>}
                                    {debtor.address && <p><span className="font-bold">Address:</span> {debtor.address}</p>}
                                    <p><span className="font-bold">Cédula:</span> {debtor.cedulaIdentidad}</p>
                                    <p><span className="font-bold">Amount Owed:</span> ${debtor.amountOwed}</p>
                                </div>
                                <div className="flex flex-col items-end gap-2">
                  <span className={`px-2 py-1 rounded text-white text-xs font-bold ${
                      debtor.status === 'PENDIENTE' ? 'bg-[#FFFF76] text-[#443CA3]' :
                          debtor.status === 'EN_GESTION' ? 'bg-[#CCE8FF] text-[#443CA3]' :
                              debtor.status === 'ACUERDO_DE_PAGO' ? 'bg-[#443CA3]' :
                                  debtor.status === 'PAGADO' ? 'bg-[#21FE83] text-[#443CA3]' :
                    ...
                    {debtor.status.replace(/_/g, ' ')}
                                    ...
                                    <div className="space-x-3 text-sm">
                                        <button onClick={() => handleEdit(debtor)} className="text-[#443CA3] hover:underline">Edit</button>
                                        <button onClick={() => handleDelete(debtor.id)} className="text-red-500 hover:underline">Delete</button>
                                    </div>
                                </div>
                            </li>
                        ))}
            ...
            <button onClick={() => signOut({ callbackUrl: '/login' })}
                    className="mt-12 bg-[#443CA3] text-white px-6 py-3 rounded-xl shadow hover:bg-[#352f85] transition">
                Logout
            </button>
        </div>
    );
